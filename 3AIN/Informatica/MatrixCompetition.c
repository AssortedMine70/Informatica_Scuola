...
void SpyralMatrix(int mat[][N], unsigned n) { // Stampa la Spyral Matrix in funzione della sua dimensione.
	unsigned num = 0;
	for(unsigned k = 0; k<n; ++k) {
		for(unsigned i = k; i < n-k; ++i)
			mat[k][i] = ++num;
		for(unsigned i = k+1; i < n-k; ++i)
			mat[i][n-k-1] = ++num;
		for(unsigned i = n-k-1; i-->k; )
			mat[n-k-1][i] = ++num;
		for(unsigned i = n-k-1; i-->k+1; )
			mat[i][k] = ++num;
	}
}
