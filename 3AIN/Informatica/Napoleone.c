...
//#define DEBUG		// if defined, writes in nextmove.txt a possible move.
//#define AUTOMATE	// if defined, the game will run automatically. DEBUG needs also to be defined.
...
void Game_Movement(size_t *x2, size_t *y2, size_t *x1, size_t *y1, struct Stack Table[4][3]) {
	gotoxy(0, TABLE_Y+30);
...
//								if(Game_CeckUnderCard(Table, x1, y1)||x2==1) {
#ifdef DEBUG
									FILE *fp = fopen("nextmove.txt", "w");
									fprintf(fp,"%u %u %u %u", (unsigned)x1, (unsigned)y1, (unsigned)x2, (unsigned)y2);
									fclose(fp);
#endif
									return 1;
								}
...
#ifdef AUTOMATE
		unsigned x2, y2, x1, y1;
		FILE *fr = fopen("nextmove.txt", "r");
		fscanf(fr,"%u %u %u %u", &x1, &y1, &x2, &y2);
		fclose(fr);
#else
		size_t x2, y2, x1, y1;
		Game_Movement(&x2, &y2, &x1, &y1, Table);
#endif
