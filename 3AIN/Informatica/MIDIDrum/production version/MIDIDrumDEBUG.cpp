// this version of the file contains various defines useful for debug.
#include <stdio.h>
#include <conio.h>
#include <dirent.h>
#include <sys/stat.h>
...
#define SCREENRES		// if defined, prints in stdout the screen resolution.
#define MIDIDEBUG		// if defined, prints in stdout MIDIOUT devices list and midi-related errors.
#define CLICKMARK		// if defined, leaves a red pixel where the mouse pressed.
#define LOG				// if defined, prints in stdout coordinates (absolutes and relatives) of the click and, if in sandbox mode, the drum part pressed, if pressed.
#define HITBOXES		// if defined, draws rectangles that rapresents clickable areas of the sandbox drum.
#define PEDALDEBUG		// if defined, prints in stdout pedal changes in sandbox mode.
#define FILEPREW		// if defined, prints in stdout a preview of what has been read from file in music sheet mode.
#define METRODEBUG		// if defined, prints in stdout every time the metronome plays.
#define TIMEDEBUG		// if defined, prints in stdout every time the notetime changes and in what changed to.
#define LENDEBUG		// if defined, prints in stdout every time the Sheet lenght changes and in what changes to.
#define TICKDEBUG		// if defined, prints in stdout wich Unit is playing.
...
#ifdef CLICKMARK
	// leaves a red pixel where the mouse pressed.
	putpixel(x, y, RED);
#endif
#ifdef LOG
	// prints in stdout coordinates (absolutes and relatives) of the click and the drum part pressed, if pressed.
...
#ifdef HITBOXES
	// draws rectangles that rapresents clickable areas
	rectangle(width*0.22, height*0.15, width*0.41, height*0.22);
	rectangle(width*0.12, height*0.32, width*0.29, height*0.38);
	rectangle(width*0.62, height*0.27, width*0.85, height*0.36);
	rectangle(width*0.36, height*0.33, width*0.48, height*0.41);
	rectangle(width*0.49, height*0.33, width*0.63, height*0.42);
	rectangle(width*0.27, height*0.44, width*0.44, height*0.52);
	rectangle(width*0.57, height*0.45, width*0.73, height*0.51);
	rectangle(width*0.43, height*0.65, width*0.51, height*0.87);
#endif
...
#ifdef PEDALDEBUG
				if(hithatpedal)
					printf("PEDAL CHANGE: Started Pressing Hit-Hat Pedal.\n");
				else
					printf("PEDAL CHANGE: Hit-Hat Pedal Released.\n");
#endif
			} else if(input==32) { // SPACE = 32
#ifdef PEDALDEBUG
				printf("PEDAL CHANGE: Bassdrum Pedal Pressed.\n");
#endif
				// play bassdrum
				// variable which is both an integer and an array of characters, used as MIDI message:
				union { unsigned long word; unsigned char data[4]; } message;
...
void sheetClickHandler(int x, int y) {
	...
	if(y>yoffset && y<yoffset+block*8 && x>xoffset && x<xoffset+(int)sheetlen*block) {
...
#ifdef LOG
		printf("valid mouse click at %i, %i.\n", x, y);
		printf("Selected Square: %i %i\n", row, col);
#endif
		int points[8];
		...
		if(getpixel(points[0], points[1])!=YELLOW) {
			...
			setfillstyle(SOLID_FILL, YELLOW);
#ifdef LOG
			printf("Fill Color: YELLOW\n");
#endif
		} else {
			setcolor(BLACK);
			setfillstyle(SOLID_FILL, BLACK);
#ifdef LOG
			printf("Fill Color: BLACK\n");
#endif
		}
		fillpoly(4, points);
	}
...
bool musicSheet() {
	...
	rewind(fp);
#ifdef FILEPREW
	// prints in stdout a preview of what has been read from file
	printf("file length: %u\n", (unsigned)sheetlen);
	for(size_t j = 0; j<sheetlen; ++j) {
		for(size_t k = 0; k<sizeof(Unit); ++k)
			if(Sheet[j].part[k]==true)
				printf("X");
			else
				printf("0");
		printf("\n");
	...
	while(input!=27) { // ESC = 27
		...
		if(kbhit()>0)
			...
			if(input==109) {// M = 109
...
#ifdef METRODEBUG
				if(metronome)
					printf("Metronome Turned ON.\n");
				else
					printf("Metronome Turned OFF.\n");
#endif
			} else if(input==32) {
				play=!play;
				tick = 0;
				...
				input = 0;
#ifdef TIMEDEBUG
				printf("speed up, notetime = %i\n", notetime);
#endif
			} else if(input==45) { // ARROW DOWN = 80
				notetime+=SPEEDSENSIBILITY;
				input = 0;
#ifdef TIMEDEBUG
				printf("speed down, notetime = %i\n", notetime);
#endif
				...
				drawSheet();
#ifdef LENDEBUG
					printf("sheet lenght decreased: %i\n", sheetlen);
#endif
				...
				drawSheet();
#ifdef LENDEBUG
					printf("sheet lenght increased: %i\n", sheetlen);
#endif
			}
		}
